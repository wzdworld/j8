package com.star.mkdocshelper.controller.readthedocs;
 ...
 * 处理readthedocs系列的操作
 */
public class ReadthedocsController {
    /**
     * 在项目文件夹里执行创建readthedocs系列的操作
     */
    public void createReadthedocs(String siteName, MainWindowController mainWindowController) {
        // 初始化.helper文件夹
        initHelperFolder();
        ...
        createCommandList.addAll(CommandListConstant.READTHEDOCS_CREATE_WITHOUT_CD_COMMAND_IN_HEADER);
        ...
        CommandUtil.executeBatFileShowWindow(batPath, (output) -> {
            ...
            // 生成配置文件
            generateReadthedocsConfigFile(siteName);
            // 刷新左侧列表
    ...
    private void generateReadthedocsConfigFile(String siteName) {
        // 处理创建失败逻辑

        // 删除配置文件
        FileUtil.deleteFile(GlobalVariables.projectPath + "/mkdocs.yml");
        // 重置Java对象
        Readthedocs.resetConfigInstance();
        // 创建Java对象
        Readthedocs readthedocs = Readthedocs.getConfigInstance();
        // 设置站点名字
        readthedocs.setSite_name(siteName);

        // 使用工具将Java对象转换成YAML文件
        SerializationUtil.javaToYaml(readthedocs, GlobalVariables.projectPath + "/mkdocs.yml");
    ...
    private void initHelperFolder() {
        ...
        FileUtil.createFileWithContent(GlobalVariables.projectPath + "/.helper/category.txt", WebCategoryConstant.READTHEDOCS);
    }
